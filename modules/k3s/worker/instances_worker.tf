...
resource "null_resource" "worker_eip" {
  ...
  provisioner "remote-exec" { 
    inline = [
      "sudo sed -i s/${local.master_ip_prev}/${local.master_ip}/g /etc/rancher/k3s/config.yaml",
      "sudo sed -i s/-ip:.*/\"-ip: ${aws_eip.worker_with_eip[each.key].public_ip}\"/g /etc/rancher/k3s/config.yaml",
...
resource "null_resource" "stop_all_nodes" {
  ...
  depends_on = [null_resource.worker_eip]
   provisioner "local-exec" {
   command = "chmod +x ../install/k3s_stop_start_instance.sh"
  }
  provisioner "local-exec" {
    command = "../install/k3s_stop_start_instance.sh stop ${var.resource_name}"
  }
}
...
resource "null_resource" "start_server1_server2" {
  ...
  provisioner "local-exec" {
    command = "../install/k3s_stop_start_instance.sh start_s1_s2 ${var.resource_name}"
  }
}
...
resource "null_resource" "start_master_worker" {
  ...
  provisioner "local-exec" {
    command = "../install/k3s_stop_start_instance.sh start_master_worker ${var.resource_name}"
  }
}